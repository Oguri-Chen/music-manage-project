  ...
  <el-card>
    ...
    <el-table :data="tableData" stripe class="table">
      <el-table-column v-for="(item, index) in options" :prop="item.prop" :label="$t(`table.${item.label}`)"
        ...
        <template v-slot="{ row }" v-if="item.prop === 'sex'">
          <svg-icon @click="changeSex(row)" :icon="row.sex ? 'man' : 'woman'"></svg-icon>
        </template>
        <!-- <template v-slot="{ row }" v-if="item.prop === 'isVip'">
          <el-switch v-model="row.isVip" @change="changeInfo(row)"></el-switch>
        </template> -->
        <template v-slot="{ row }" v-if="item.prop === 'vipExpiration'">
          <p v-if="row.isVip">{{ moment(row.vipExpiration).format('YYYY-MM-DD') }}</p>
          <p v-else>非vip</p>
        </template>
